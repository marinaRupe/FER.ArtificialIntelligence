...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        maxQValue = self.getQValue(state, actions[0])
    ...
    def update(self, state, action, nextState, reward):
        ...
        self.QValues[state, action] = QState + self.alpha * (reward + self.discount * maxQNextState - QState)
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        QState = self.getQValue(state, action)
        maxQNextState = self.computeValueFromQValues(nextState)
        difference = (reward + self.discount * maxQNextState) - QState
        featureVector = self.featExtractor.getFeatures(state, action)
